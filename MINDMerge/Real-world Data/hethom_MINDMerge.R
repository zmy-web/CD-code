...
# filepath = paste('./hospital/',hospitalID,'_c_train.csv',sep = '')
# full_data = read.csv(filepath,header = TRUE)
# cols <- sapply(full_data, is.numeric)
# listA = which(colSums(full_data)==0)
# for(i in listA){
#   A = names[i]
#   full_data[,A]<- factor(full_data[,A],levels = c(0,1))
# }
# full_data[,cols] <- lapply(full_data[,cols], as.factor)
# 
...
for(NO in 1:50){
  ...
  # filepath1 = paste('./hospital/',hospitalID,'_total',sep = '')
  ...
  for(method in 1:5){
    ...
    if(method == 1){
      xval=tabu(dataset,score = 'bde',blacklist = blackList)
    }
    if(method == 2){
      xval=tabu(dataset,score = 'bdla',blacklist = blackList)
    }
    if(method == 3){
      xval=tabu(dataset,score = 'bic',blacklist = blackList)
    }
    if(method == 4){
      xval=tryCatch({
  ...
  filepath2 = paste('./result/',hospitalID,'_10_result_',sep='')
  ...
  filepath2 = paste('./result/',hospitalID,'_10_efbn_',sep='')
  filepath1 = paste(filepath2,NO,'_',sep='')
  filepath = paste(filepath1,num,'.csv',sep = '')
  write.csv(matrix_update,filepath,row.names = TRUE,col.names =TRUE,sep='')
}
}