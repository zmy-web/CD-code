...
for dataNO in range(1,2):
    # result_k2 = pd.read_csv(f'C:\\Users\\riyang\\Documents\local\\result\\{hospitalID}_efbn_1_{dataNO}.csv',index_col=0)
    result_k2 = pd.read_csv(f'C:\\Users\\riyang\\Documents\local\\result\\hethom_t_efbn_1.csv',index_col=0)
    ...
    for NO in range(1,51):
        tmp = pd.read_csv(f'C:\\Users\\riyang\\Documents\local\\result\\hethom_t_efbn_{NO}.csv',index_col=0)
        total_result += tmp
    ...
    dict1 = {}
    num = 1
    df = pd.DataFrame({'lamb1': ['1212'], 'lamb2': ['1212'], 'edges': ['122']})
    for lamb2 in np.arange(0.2, 3, 0.2):
        for lamb1 in np.arange(0.1, 5, 0.1):
            try:
                W_est = notears_linear(X,lambda1=lamb1,lambda2=lamb2, loss_type='l2')
            except:
                continue
            ...
            for i in range(length):
                for j in range(length):
                    if(res.iloc[i,j] > res.iloc[j,i]):
                        ...
                        res.iloc[i,j] = 0
            # res.to_csv(f'{alarm}_res{dataNo}.csv')
            ...
            print(count)
            df.loc[len(df)] = [lamb2,lamb1,count]
            arcs_num.append(count)
    ...
    df.to_csv(f'{hospitalID}_{dataNO}_t_parameter.csv', index=False)
    with open(f'C:\\Users\\riyang\\Documents\\local\\{hospitalID}_{dataNO}_t_p12_file.json', 'w',
    ...
    #     for i in range(len(names)):
    #         if (row[i] > 0):
    #             graph.add_node(pydot.Node(names[i], shape="oval",color = 'red',style='filled'))
