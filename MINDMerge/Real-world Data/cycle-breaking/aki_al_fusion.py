...
dict1 = {}
hospitalID = 435
for dataNo in range(1,6):

        tmp = pd.read_csv(f'C:\\Users\\riyang\\Documents\\local\\result\\{dataNo}mimic_mix_efbn.csv', index_col=0)
        ...
        graph.del_edge('tub', 'either')
        graph.add_edge(pydot.Edge('tub', 'either', color='#349E43'))

        graph.del_edge('either', 'xray')
        graph.add_edge(pydot.Edge('either', 'xray', color='#349E43'))

        graph.del_edge('either', 'dysp')
        graph.add_edge(pydot.Edge('either', 'dysp', color='#349E43'))
    ...
    #
with open(f'C:\\Users\\riyang\\Documents\local\\mimic_het_mix_file.json', 'w', encoding='utf-8')as file:  
