...
for(dataNO in 1:10){
  print('dataNO:')
  print(dataNO)
  filepath = paste('select/child_5000_',dataNO,'.csv',sep='')
  ...
  for(NO in 1:50){
    ...
    for(method in 1:5){
      ...
      if(method == 5){
        xval=tryCatch({
          mmhc(dataset,maximize.args = list(score = 'k2'))
          },error=function(e){
            mmhc(dataset)
          })
      }
    ...
    filepath2 = paste('./result/',dataNO,'_child_5000_result_',sep='')
    filepath1 = paste(filepath2,NO,'.csv',sep='')
    write.csv(matrix_sum_one,filepath1,row.names = TRUE,col.names =TRUE,sep='')
    filepath2 = paste('./result/',dataNO,'_child_5000_efbn_',sep='')
    filepath1 = paste(filepath2,NO,'.csv',sep='')
